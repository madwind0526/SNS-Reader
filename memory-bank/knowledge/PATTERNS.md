...
- Mesh View node size should scale by post connection degree, while the right Top TAG panel can remain as a textual summary. Card Filter can reuse the same non-platform TAG connection count as a min/max range filter.
...
- Instagram/Threads post images should be captured by navigating to the post's own permalink and reading `og:title`/`og:description` (caption) + `og:image` + `<time datetime>` (exact timestamp) - all meant for link-preview crawlers/SEO, so they are far more stable against a UI redesign than scraping rendered DOM text. A profile feed page's `document.body.innerText` can be too sparse (Instagram) or too brittle to line-parse (Threads) to use directly.
- To tell "a real post photo" apart from "the profile-picture fallback shown as og:image on a text-only post," decode the image URL's base64 `efg` query parameter (`{"efg_tag": "FEED.best_image_urlgen..."}` vs `{"efg_tag": "profile_pic.django..."}`) rather than guessing which CDN bucket ID means what - bucket IDs vary by post/media type (photo vs. reel vs. carousel item) in ways a fixed allowlist misses.
- Facebook has no reliable per-post image source while logged in (empty `og:*` meta tags, and the feed's compact article snippet never includes the real `<img>`). Re-requesting the same permalink with Facebook's own crawler user-agent (`facebookexternalhit/1.1`, no login) returns clean `og:image`/`og:description` for a public post - the same mechanism Messenger/Slack/WhatsApp use to generate their own link previews.
- Threads splits long posts into a main post plus the author's own reply/replies (character-limit workaround). Each part's own permalink page renders the whole chain and lists every part's `/post/{id}` link somewhere on the page - collect those sibling ids per captured post, then group connected posts (by shared sibling ids) and merge each group into one post (earliest part's identity, bodies joined chronologically, images combined) instead of writing each part as a separate, often imageless, card.
- A platform whose feed only exposes relative time text ("1일", "2일" - no exact machine-readable timestamp anywhere in the DOM) will compute a slightly different absolute date across two runs of the same post, depending on exactly when each run happens. Never derive a stable content ID from that computed date; derive it from the post's own permalink id (shortcode/pfbid/etc.) instead, exactly like a platform with a real `<time datetime>` element already does with its own shortcode.
- Grid/list virtualization can be implemented without a library when the grid's row height and column count are both effectively fixed (check CSS media queries against the app's actual min-width constraints - a fixed-size Electron window often never reaches a "responsive" breakpoint in practice). Render only the visible-row window plus overscan, and use two spacer elements (`grid-column: 1 / -1`) to preserve scroll height and item position - see `progress.md` for the full writeup and the polling-sync pitfall it surfaces (a "sync selected item with the refreshed list" effect can silently blank out already-loaded detail content once the list payload is trimmed).
